...
lowcut  = 500000 - 100 
highcut = 500000 + 100
sos_carrier_ae = signal.iirfilter(17, [lowcut, highcut], rs=60, btype='band',
                     analog=False, ftype='cheby2', fs=Fs,
                     output='sos')
# 
# ae_broadband_array      = np.zeros( (numpoints_x,numpoints_y,N) )
# ae_carrier_filtered = signal.sosfilt(sos_carrier_ae, data[ae_channel])  
# 40,40,500000
# use the broadband array to obtain the carrier signal. 
a,b,c = ae_broadband_array.shape
print ('ae array shape',a,b,c)
ae_carrier_array      = np.zeros( (a,b,c) )
for i in range(a):
    for j in range(b):
...
max_level = np.max(ae_array[20,20,:])
print ('max level',max_level)

idx = 95535 
mean_level = np.mean(ae_array[:, :, idx])
print ('mean level:',mean_level)
...
# elev_min = -0.11
factor = 1.0
elev_max = max_level*factor
elev_min = -max_level*factor
...
im_h = plt.imshow(ae_array[:, :, idx]-mean_level,cmap=colormap_choice,interpolation='nearest',extent=rectangle,clim=(elev_min, elev_max), norm=MidpointNormalize(midpoint=mid_val,vmin=elev_min, vmax=elev_max))
