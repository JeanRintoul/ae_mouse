...
import pandas as pd 
myFile  = pd.read_csv('AEMAP.csv', sep=',')
data    = myFile.to_numpy()[:,0:4]
mm_AP   = data[0,:]
data    = data[:,1:]
a,b     = data.shape 
print (data,a,b)
...
for i in range(a):
    for j in range(b):
        ...
        points = list(map(float, point))
        data[i,j] = np.median(points)
...
ax.axis('off') 
plot_filename = 'saline_calibration_map.png'
...
# file_number           = 28 # vep with pressure. 
