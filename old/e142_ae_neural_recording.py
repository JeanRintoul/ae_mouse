...
tt = 1 
# 
current_signal_frequency 	= 10  # This is set up for a 4Hz VEP. 
multifile_start 			= 1 
multifile_end   			= 16
gain                		= 5000
Fs                  		= 2e6  # 2e6
...
# 
# 0-1 take the whole time duration, as the kaiser window attenuates both ends. 
start_time  = 0.05
end_time 	= 1
# start_time = np.round(0.8/duration ,2)
# end_time   = np.round((duration - 0.2)/duration,2)
...
# 'long_recording': 30, 	      # ensure NO RAMP when using this. 
'duration': duration, 
'position': test_no,
...
'save_folder_path':'D:\\ae_mouse\\e142_ae_neural_decoding',
...
for i in x:
	...
	if (iteration_number%calculate_nth == 0 or i == (multifile_end-1) or iteration_number ==1 ):
		...
		# 
		c_idx = find_nearest(frequencies,carrier)
		print ('amplitude of carrier:',fft_mk[c_idx],fft_m[c_idx])
		...
		ax3 = fig.add_subplot(413) 		# check if it is railing or not? 
		# plt.plot(t,fsignal,'k')
		plt.plot(t,data[m_channel],'r')
		ax4 = fig.add_subplot(414) 	
		plt.plot(frequencies,fft_m,'k')
		ax4.set_xlim([carrier-bandwidth_of_interest,carrier+bandwidth_of_interest])
		plt.show()
		# 
...
# 
# 
outfile = savepath + '\\e140_t'+str(tt)+'.npz'
np.warnings.filterwarnings('ignore', category=np.VisibleDeprecationWarning)   
np.savez(outfile,rf_summation=rf_summation,marker_summation=marker_summation,average_signal=average_signal,n=iteration_number)
