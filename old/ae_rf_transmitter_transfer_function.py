...
# 
# 
# frequencies_list = list(np.linspace(30,int(2e6),num=30))
# 
frequencies_list = [10,100,1000,10000,100000,500000,1000000,1500000,2000000]
# 
...
for i in range(len(frequencies_list)): 
  ...
  aeti_variables['ti_frequency'] = f + dfx
  ...
  df_measured = fft_m[df_idx]  
  #
  print ('f/amplitude_measured:',f,amplitude_measured,df_measured)
  data_response.append([f,amplitude_rf,amplitude_measured,df_measured])
...
ax.set_xlim([0,2000010])
ax.set_ylabel('Volts (V)')
plt.legend(['measured df@electrode'],loc='upper right')
ax2.set_ylabel('normalized')
ax2.set_xlabel('Frequency(Hz)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
plt.suptitle('RF versus measured carrier response shape')
plot_filename = savepath + '\\t'+str(test_no)+'_df_response.png'
plt.savefig(plot_filename)
plt.show()
# 
# 
