...
sos_lowpass  = iirfilter(17, [100], rs=60, btype='lowpass',
...
'duration': duration, 
'position': test_no,
'pressure_amplitude': 5,      	# how much is lost through skull??? 400kPa, 0.08 is about 200kPz. 0.15 is about 400kPa. 
...
# 'pressure_burst_length': 0.0,  	# burst length in seconds.       
...
#
filtered_signal   	= sosfiltfilt(sos_lowpass, fsignal) 
#
fft_m  				= fft(fsignal[start_pause:end_pause])
fft_m  				= np.abs(2.0/(end_pause-start_pause) * (fft_m))[1:(end_pause-start_pause)//2]
prf_idx 			= find_nearest(frequencies,prf)
print('fft m at prf amplitude: ',fft_m[prf_idx])
# 
...
# ax2.set_xlim([carrier - 50,carrier + 50])
