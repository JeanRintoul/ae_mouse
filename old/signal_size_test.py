...
'save_folder_path':'D:\mouse_aeti\e69_mouse_4\saline_control_data',
...
fft_m = fft(data[m_channel][start_pause:end_pause]/gain)
...
sos = signal.iirfilter(17, [low_cut, high_cut], rs=60, btype='band',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
filtered_data = signal.sosfiltfilt(sos, fsignal)

# this is getting the marker signal peak... not good. 
...
# print ('filtered data pp amplitude', 2*filt_pp, 1e6*2*filt_pp)
...
# plt.plot(t, 1e6*fsignal, color = 'purple')
plt.ylabel('Volts ($\mu$V)')
# plt.plot(t, fsignal, color = 'blue')

 
plt.legend(['raw measured data' ],loc='upper right', fontsize = 8)
# ax.set_xlim([0,8])
plt.xlim([4.00,4.01])
# plt.xlim([4.00,4.8])
...
plt.plot(frequencies, 1e6*fft_m, color = 'purple')
plt.ylabel('Volts ($\mu$V)')
...
plt.plot(t, 1e6*filtered_data, color = 'orange')
plt.xlim([4.00,4.01])
# plt.plot(t,current_filtered_data,color='r')
# plt.plot(t, v_data, color = 'b',linewidth=2)
plt.legend(['filtered data ($\mu$V)'],loc='upper right', fontsize = 8)
...
plot_filename = savepath + '\\t'+str(test_no)+'_signal size test.png'
