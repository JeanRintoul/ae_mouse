...
'current_amplitude': 0.6,   #  its actually a voltage .. Volts. 
'current_frequency': 8000,
'ti_frequency': 8003,          # if this is included or  > 0 it means we are adding two sine waves together. i.e. TI. 
...
'save_folder_path':'D:\mouse_aeti\e62_mouse_1\q2\\c_ti',
}
#  
...
plt.xlabel('time(s)')
plot_filename = savepath + '\\'+str(test_no)+'lfp.png'
...
ax.spines['top'].set_visible(False)
# plt.xlabel('time(s)')
plot_filename = savepath + '\\'+str(test_no)+'lfp_plot.png'
plt.savefig(plot_filename)
plt.show()
...
ax.set_xlim([0,21000])
# plt.xlabel('time(s)')
plot_filename = savepath + '\\'+str(test_no)+'artefact_frequency.png'
plt.savefig(plot_filename)
plt.show()