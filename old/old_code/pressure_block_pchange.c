...
int main(int argc, char* argv[])
{
  ...
  char pressure_name[MAX];
  double pressure_amplitude         = 0;  // this is the output voltage of the tiepie.
  if(argc==1)
      printf("\nNo Extra Command Line Argument Passed Other Than Program Name");
  if(argc>=2)
  {
    ...
    for(int counter=0;counter<argc;counter++)
    {
      ...
      { 
        ...
        sscanf(argv[2],"%lf\n",&pressure_amplitude); 
        printf("\npressure_amplitude(Hz): %s\n",argv[2]);        
        // snprintf(pressure_name, MAX, "%s", (int)(pressure_amplitude*100) ) ;
        snprintf(pressure_name, MAX, "%s", argv[2]) ;
  ...
  else if(ScpIsDataReady(scp))
  {
    ...
    char file_ending[25] = "_pressure_block.npy";
    char file_label[100];
    strcpy(file_label,file_ending);
    strcat(pressure_name,file_label);
    printf("\nfilename result: %s\n",pressure_name);

    // Write .npy file
    // const char* filename = "current_check_block.npy";
    FILE* f = fopen(pressure_name, "wb");
    skip_npy_header(f);
