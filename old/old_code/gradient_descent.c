...
int main(int argc, char* argv[])
{
  ...
  double current_amplitude         = 0;  // this is the output voltage of the tiepie.
  ...
  // 2. hydrophone probe
  // 3. current monitor for e2
  // 4. tiepie voltage output waveform
  // 5. voltage monitor for e1 x10
  // 6. voltage monitor for e2 x10
  // 7. current monitor for e1
  // 8. diff input voltage across 1k resister.
  // 
  ...
  double dRanges[8] = {2.0,2.0,2.0,0.2,0.4,2.0,2.0,2.0};
  ...
  // Set frequency:
  GenSetFrequency(gen_current, 12000); // 500kz  1 kHz
  CHECK_LAST_STATUS();
  ...
  else if(ScpIsDataReady(scp))
  {
    ...
    // Write .npy file
    const char* filename = "gdescent_block.npy";
    ...
    // Find the max and min, then use this to calculate the mA range. 
    // float lastmax = 0;
    // float lastmin = 0;
    float cumsum = 0.0;

    float resistor_current_mon = 49.9;
    for(uint64_t i = 0; i < record_length; i++)
    {
      for(uint16_t ch = 0; ch < channel_count; ch++)
      {
        if (ch == 3)
        {
          cumsum = cumsum + fabs(channelData[ch][i]);
          // printf(";%f", channelData[ch][i]);
          // if (channelData[ch][i] > lastmax)
          // {
          //   lastmax = channelData[ch][i];
          // }
          // if (channelData[ch][i] < lastmin)
          // {
          //   lastmin = channelData[ch][i];
          // }  
        }
      }
    }
    // In a sine wave VAV = VPK x 0.637 = VPK * (2 / PI)
    // average = cumsum/(record_length)
    // VPK = average*PI/2
    // VPP = 2* average*PI/2 
    // VPP = average*PI = cumsum*PI/record_length
    // float average = cumsum/record_length;
    // printf("Vpp : %f %f %f\n",average,cumsum,(float)record_length);
    // Now converting this to curent using our known value resistor. 
    printf("current max(mA): %f", ((1000*5*(cumsum)*PI)/record_length)/resistor_current_mon );
    // printf("current max(mA): %f", (1000*5*(lastmax-lastmin))/resistor_current_mon );
