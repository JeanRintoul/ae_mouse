...
int main(int argc, char* argv[])
{
  ...
  while ((opt = getopt(argc, argv, "f:g:p:d:s:a:b:t:e:")) != -1) {    
      switch(opt) {
      case 'f':
      ...
      case 't':  // start pause in pecentage of total samples
        ...
        printf("\nstart_pause: %s\n",optarg);
      break;  
      case 'e':  // start pause in pecentage of total samples
        sscanf(optarg,"%lf\n",&end_pause);
        printf("\nend_pause: %s\n",optarg);
      break;              
      case '?':
...
// Define the constants used. 
  ...
  double dRanges[8] = {20.0,0.2,20.0,20.0,0.4,0.4,2.0,12.0};  
