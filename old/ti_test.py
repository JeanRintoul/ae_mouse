...
# 'current_amplitude': 1.0,   #  its actually a voltage .. Volts. 0.0002
'current_amplitude': 0.05,
'current_frequency': 1000.0,
#'ti_frequency':2001,       # if this is included or  > 0 it means we are adding two sine waves together. i.e. TI. 
...
'save_folder_path':'D:\\mouse_aeti\\e77_vep_test',
...
low_cut  = f-0.5  # where do papers normally put this cut-off? 
if low_cut <= 0:
   low_cut = 0.05
high_cut = f+400
...
low_cut  = 0.1 # where do papers normally put this cut-off? 
high_cut = 45.0

sos = signal.iirfilter(17, [low_cut, high_cut], rs=60, btype='band',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
mfiltered_data = signal.sosfiltfilt(sos, data[m_channel])


ilow_cut = current_signal_frequency - 100 
if ilow_cut < 0:
   ilow_cut = 0.01
ihigh_cut = current_signal_frequency +100
isos = signal.iirfilter(17, [ilow_cut, ihigh_cut], rs=60, btype='band',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
current_filtered_data = signal.sosfiltfilt(isos, i_data)
...
# plt.plot(t, data[m_channel]/np.max(data[m_channel]), color = 'purple')
# # plt.plot(t, sine_wave, color = 'red')
# plt.plot(t, mfiltered_data/np.max(mfiltered_data), color = 'orange')
plt.plot(t, data[m_channel], color = 'purple')
# plt.plot(t, sine_wave, color = 'red')
plt.plot(t, mfiltered_data, color = 'orange')
...
plt.legend(['measured data','applied voltage'],loc='upper right', fontsize = 8)
...
plt.plot(t, v_data, color = 'purple')
...
plt.legend(['v chan','measured data'],loc='upper right', fontsize = 8)
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
...
# plt.plot(t, current_filtered_data/np.max(current_filtered_data), color = 'g')
