...
prf 						= 80
c = carrier 
# carrier_list 				= [c,c-prf,c+prf,c-2*prf,c+2*prf,c-3*prf,c+3*prf,c-4*prf,c+4*prf ]
carrier_list 				= [c,c-prf,c+prf,c-2*prf,c+2*prf]
#
...
'save_folder_path':'D:\\ae_mouse\\e136_ae_neural_decoding',
...
for i in x:
	...
	# 
	# Implement PRF averaging. 
	demodulate_signal_sum 		= [0]*t
	single_demod 				= [0]*t
	total_n = len(carrier_list)
	for j in range(len(carrier_list)):
		current_carrier   	= carrier_list[j]
		print ('current_carrier',current_carrier,j)
		sos_prf_bandpass  = iirfilter(17, [current_carrier - bandwidth_of_interest,current_carrier + bandwidth_of_interest], rs=60, btype='bandpass',
	                       analog=False, ftype='cheby2', fs=Fs,
	                       output='sos')
		filtered_signal   = sosfiltfilt(sos_prf_bandpass, average_signal) 
		demodulate_signal = demodulate(filtered_signal,carrier,t)
		demodulate_signal = sosfiltfilt(sos_lowpass,demodulate_signal)
		if j == 0:
			demodulate_signal_sum = demodulate_signal
			single_demod = demodulate_signal
		else: 
			demodulate_signal_sum = demodulate_signal_sum + demodulate_signal
	averaged_single_iteration_demod = demodulate_signal_sum/total_n

	# now average over multiple recordings. 
	summed_demod  = summed_demod + averaged_single_iteration_demod
	average_demod = summed_demod/(iteration_number)
	# 
	...
	if (iteration_number%calculate_nth == 0 or i == (multifile_end-1) ):
		...
		# 
		fft_d  = fft(average_demod[start_pause:end_pause]*window)
		fft_d  = np.abs(2.0/(end_pause-start_pause) * (fft_d))[1:(end_pause-start_pause)//2]
		fft_sd = fft(single_demod[start_pause:end_pause]*window)
		fft_sd = np.abs(2.0/(end_pause-start_pause) * (fft_sd))[1:(end_pause-start_pause)//2]
		# 
		# Plot the calculated FFT. 
		fig = plt.figure(figsize=(5,5))
		ax = fig.add_subplot(211)
		plt.plot(frequencies,fft_mk,'k')
		ax.set_xlim([carrier-1000,carrier+1000 ])
		...
		plt.plot(frequencies,fft_sd,'r')
		plt.plot(frequencies,fft_d,'k')
		ax2.set_ylim([0,1])
		ax2.set_xlim([0,bandwidth_of_interest])
		plt.legend(['single','av demod fft signal'],loc='upper right')
		plt.show()
		# 
...
fft_m = np.abs(2.0/(end_pause-start_pause) * (fft_m))[1:(end_pause-start_pause)//2]
# 
fft_d = fft(average_demod[start_pause:end_pause])
fft_d = np.abs(2.0/(end_pause-start_pause) * (fft_d))[1:(end_pause-start_pause)//2]
# 
...
ax = fig.add_subplot(111)
plt.plot(frequencies,fft_d,'k')
plt.legend(['average demod'],loc='upper right')
plt.show()
