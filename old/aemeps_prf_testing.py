...
# 
test_no = 1  # 
gain    = 1  # Change this difference frequency. 
dfx     = 10
...
#'current_ISI':1.5,   # use this to add a pause in the middle if I want. 
'current_ISI':0,   # use this to add a pause in the middle if I want. 
...
if df_l < 0:
    ...
    sos_df_band = iirfilter(17, [df_l,df_h], rs=60, btype='bandpass',
                           analog=False, ftype='cheby2', fs=Fs,
                           output='sos')
# emg_df_data     = sosfiltfilt(sos_df_band, v_data)
...
plt.legend(['measurement data'],loc='upper right')
