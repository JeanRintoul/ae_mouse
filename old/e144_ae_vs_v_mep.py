...
# 
# pressure should be twice as fast as voltage. 
# 
gain                = 500
Fs                  = 2e6
carrier             = 500000
...
sos_emg_band    = iirfilter(17, [40,800], rs=60, btype='bandpass',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
...
# 
# 
# For this, I want the current to slowly ramp.  

print ('start and end',start_time,end_time)

current_ISI  = 2.0 
pressure_ISI = 2.0 * current_ISI
# 
current_amplitude  = 6.0
pressure_amplitude = 0.4
# 
aeti_variables = {
...
'pressure_ISI':pressure_ISI,             	# inter trial interval in seconds. 
'pressure_burst_length': 0.5,  	    # burst length in seconds. 
'current_amplitude':current_amplitude,       
'current_frequency':current_signal_frequency,   # 
'current_ISI':current_ISI,  			# time in seconds between repeats. default length of time for pulse is 0.05s. see c code. 
...
'save_folder_path':'D:\\ae_mouse\\e144_meps',
...
# 
df_idx = m.find_nearest(frequencies,1)
# 
# 
print ('iso freq:',np.round(frequencies[df_idx],2 ))
print ('df:',2*fft_m[df_idx])
print ('pp height: ', np.max(low_signal)-np.min(low_signal))
...
plt.legend(['v sig'],loc='upper right')
ax2 = fig.add_subplot(512)
plt.axvline(x=frequencies[df_idx],color='r')
plt.plot(t,rfsignal,'r')
plt.legend(['rf sig'],loc='upper right')
