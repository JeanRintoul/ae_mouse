...
fig, (ax5) = plt.subplots(nrows=1, sharex=True, subplot_kw=dict(frameon=False),figsize=(12,6.0)) # frameon=False removes frames
...
# ax4.plot(t,data[ae_channel],color = tableaucolorblind10[4])
ax5.plot(t,ae_filtered*AE_gap/gain,color = tableaucolorblind10[5])
# ax6.plot(t,dc_filtered,color = tableaucolorblind10[6])
plt.xlabel('Time (seconds)',fontsize=12)  

graph_names = ['RF Amplifier','Hydrophone','Applied Voltage','Raw AE','Filtered AE(df only)','DC filtered']  
startpt = -0.1     
...
ind = np.argmax(ae_filtered,axis=0)
print ('index of max: ',ind)
vlinex2 = t[ind] # 1.7
print ('vlinex2:',vlinex2)
vlinex2 = 1.72

# ax5.annotate("",
#             xy=(vlinex1, 0.3), xycoords='data',
#             xytext=(vlinex2 , 0.3), textcoords='data', horizontalalignment="center",
#             arrowprops=dict(arrowstyle="<->"), verticalalignment = "center")

# ax5.text(vlinex1+(vlinex2-vlinex1)/2, 0.5,"dt", horizontalalignment="center",
#             verticalalignment = "center")

plt.savefig("onset-analysis.svg", bbox_inches="tight",format="svg") 
plt.show()
