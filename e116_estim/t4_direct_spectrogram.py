...
for n in range(len(file_list)):
    ...
    for i in range(len(alignment_indices)-1):  # 
        if (i+1)%2 ==0:
            baseline = np.mean(data_to_segment[(alignment_indices[i]-pre_event_idxcount):alignment_indices[i] ])
            # print ('baseline',baseline)
            segment = data_to_segment[(alignment_indices[i]-pre_event_idxcount):(alignment_indices[i]+ section_length)]-baseline
            filtered_segmented_data.append(segment)
            print ('i',i,len(filtered_segmented_data),len(segment))
