...
outfile = t_series + '-vep_10hz-shielded.npz'
# 
savepath       = 'D:\\ae_mouse\\e142_ae_neural_decoding\\t2_mouse_na\\vep_shield_test_10hz_good\\10hz_vep\\'
savepath       = 'D:\\ae_mouse\\e142_ae_neural_decoding\\t2_mouse_na\\vep_shield_test_10hz_good\\10Hz_vep_shield\\'
...
for n in range(len(file_list)):
    ...
    #                        output='sos')
    # 
    fsignal           = sosfiltfilt(sos_df_band, fsignal)
    # #
    ...
    indexes = np.argwhere(diffs > 0.15)[:,0]
    X       = np.insert(indexes, 0, 0)
    m       = np.diff(X)
    print ('m',m)
    m       = np.argwhere(m > 10)[:,0]
    ...
    # plt.plot(t,200*marker/np.max(marker),'b')    
    ...
    # d_df_data = sosfiltfilt(sos_df_band, fsignal)    
    # 
    # d_df_data = fsignal
