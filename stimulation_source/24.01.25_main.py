...
pin_event_generator  = 27 # when the generator starts. 
pin_led              = 14
...
generator_event  = Pin(pin_event_generator, machine.Pin.IN, machine.Pin.PULL_UP)
led.value(0)
led_on.value(0)
...
def pin_high_on_generator_handler(sm):
    global check
    print ('pin high handler called, ready to receive event')
    skip_one = 0
    check                = 1
...
# This waits for the pin high of the generator. 
sm3 = rp2.StateMachine(0, wait_pin_high, in_base=generator_event)
sm3.irq(pin_high_on_generator_handler)
# 
