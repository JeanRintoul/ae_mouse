...
duration   = 12.0 
start_null = 0.0 * duration * 1000 # in milliseconds(ms)
end_null   = 1.0 * duration * 1000 # in milliseconds(ms)
...
frequency              = 20    # 0.0625 duration, 1 on every 0.25s.
...
#led_stop_time       = 5000      # in milliseconds(ms)
led_stop_time        = 29500
#led_stop_time        = 11500
# led_stop_time        = 5500
# led_stop_time        = 4000
...
start_time           = 0
skip_one             = 0
pin_event            = 28 # when the daq starts
pin_led              = 14
...
# this happens when the generator starts.
def pin_low_handler(sm):
    print ('pin low handler called, daq or gen event')
    global start_time,check,skip_one
    ...
    # 1000/f 
    tim.init(mode=Timer.PERIODIC, callback=tick, period=int(1000/frequency) )
...
def pin_high_handler(sm):
    global check, skip_one
    print ('pin high handler called, reset all and ready to receive event')
    skip_one = 0
    start_time           = 0
...
def button_handler(sm):
    ...
    if check == 0:
        ...
        tim.init(mode=Timer.PERIODIC, callback=button_tick, period=int(1000/frequency) )
        start_time = time.ticks_ms()
        print(time.ticks_ms(), sm)
        led_on.value(1)
        check = 1
        
print ('clock speed that state machine is run at: ',machine.freq())
