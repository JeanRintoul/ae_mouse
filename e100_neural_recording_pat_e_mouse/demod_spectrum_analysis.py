...
# savepath        = 'D:\\mouse_aeti\\e100_neural_recording_pat_e_mouse\\t5_delta_wave_visual_cortex\\'
savepath        = 'D:\\mouse_aeti\\e100_neural_recording_pat_e_mouse\\t6_mep_delta_wave_mortorcortex\\'
...
nac_unfiltered_data = nac_fsignal
nac_rf_data = 10*nac_data[rf_channel]
# do the demodulation
nac_fraw            = sosfiltfilt(sos_carrier_band, nac_fsignal) 
nac_fraw            = sosfiltfilt(sos_center_cut, nac_fraw) 
nac_demod_data      = demodulate(nac_fraw)
nac_demodulated     = sosfiltfilt(sos_low, nac_demod_data)
nac_fsignal         = sosfiltfilt(sos_low, nac_fsignal)
# 
# 
...
rfcarrier_signal       = sosfiltfilt(carrier_filter, rf_data)
double_carrier_signal  = sosfiltfilt(double_carrier_filter, unfiltered_data)
double_rfcarrier_signal  = sosfiltfilt(double_carrier_filter, rf_data)
...
start_pause     = int(1.8*Fs) # int(0.2 * N+1)
end_pause       = int(4*Fs) # int(0.8 * N-1)
...
# nac data. 
nac_fft_demod   = fft(nac_demodulated[start_pause:end_pause])
nac_fft_demod   = np.abs(2.0/(end_pause-start_pause) * (nac_fft_demod))[1:(end_pause-start_pause)//2]
nac_fft_unfiltered_data = fft(nac_unfiltered_data[start_pause:end_pause])
nac_fft_unfiltered_data = np.abs(2.0/(end_pause-start_pause) * (nac_fft_unfiltered_data))[1:(end_pause-start_pause)//2]
# fft_data            = fft(fsignal[start_pause:end_pause])
# fft_data            = np.abs(2.0/(end_pause-start_pause) * (fft_data))[1:(end_pause-start_pause)//2]
...
plt.legend(['unfiltered_data'],loc='upper right')
plt.show()
# 
...
plt.plot(frequencies,fft_demod,'k')
# plt.plot(frequencies,fft_unfiltered_data,'k')
ax.set_xlim([0,40])
plt.legend(['ac neural signal','ac demod'],loc='upper right')
ax.set_ylabel('Volts ($\mu$V)')
# ax.set_xlabel('Frequencies(Hz)')
ax2 = fig.add_subplot(212)
plt.plot(frequencies,nac_fft_unfiltered_data,'r')
plt.plot(frequencies,nac_fft_demod,'k')
ax2.set_xlim([0,40])
ax2.set_ylabel('Volts ($\mu$V)')
ax2.set_xlabel('Frequencies(Hz)')
plt.legend(['nac neural signal','nac demod'],loc='upper right')
...
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(211)
plt.plot(t,rfcarrier_signal,'k')
plt.plot(t,double_rfcarrier_signal,color='pink',alpha = 0.8)
plt.legend(['rf monitor carrier','rf monitor sum'],loc='upper right')
ax.set_ylabel('Volts (V)')
ax.set_xlim([0,duration])
ax2 = fig.add_subplot(212)
plt.plot(t,carrier_signal,'k')
plt.plot(t,double_carrier_signal,color='pink',alpha = 0.8)
plt.legend(['m chan carrier f','m chan on sum f'],loc='upper right')
ax2.set_ylabel('Volts ($\mu$V)')
ax2.set_xlim([0,duration])
plt.suptitle('rf data file: '+str(file_number))
