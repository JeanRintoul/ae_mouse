...
#
t_series = 't3'
outfile = t_series + '-pulse_data_tight_filter.npz'
# 
savepath    = 'D:\\ae_mouse\\e126_mouse_improved_stim_setup\\t3_mouse\\'
gain        = 500
m_channel   = 0 
rf_channel  = 2
...
for n in range(len(file_list)):
    ...
    #   zeroed_fsignal[ (alignment_indices[j]):(alignment_indices[j]+int(1.1*pulse_length*new_Fs) )] = background_mean
    # fig  = plt.figure(figsize=(10,6))
    # ax   = fig.add_subplot(111)
    # plt.plot(dt,zeroed_fsignal,'k')
    # plt.title('zeroed fsignal')
    # plt.show()
    # 
    ...
    # # 
    # # mains filtering adds noise around the peaks.  
    ...
    # alignment_indices = alignment_indices[0:5]
    ...
    for i in range(len(alignment_indices)):  #
        ...
        if i % periods_of_interest == 0:
            ...
            if len(segment) == array_len: # ensure only full sections are appended.
                if (np.max(segment)-np.min(segment)) < 100: # rudimentary filter. 
...
plot_filename = t_series+'_pulse_events_'+str(n_events)+'.png'
