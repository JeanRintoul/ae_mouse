...
# file_list1   = np.arange(start,stop,step)
# file_list2   = np.arange(start2,stop2,step)
# # print ('one: ',file_list1)
# file_list = np.concatenate( (file_list1,file_list2) )
print ('file list',file_list)
#
t_series = 't1'
outfile = t_series + '-fswitch_anesthesia_data_filter.npz'
# 
savepath    = 'D:\\ae_mouse\\e126_mouse_improved_stim_setup\\t1_mouse\\'
...
m_channel   = 0 
rf_channel  = 2
pulse_length = 0.01 # pulse length in seconds. 
...
for n in range(len(file_list)):
    ...
    # 
    # 
    sos_downsampling_band = iirfilter(17, [new_Fs/2], rs=60, btype='lowpass',
                           analog=False, ftype='cheby2', fs=Fs,
                           output='sos')
    # 
    ...
    indexes = np.argwhere(diffs > 0.005)[:,0]
    ...
    # 
    # this is to remove the start and end regions which are muddied by the onset.  
    start_idx = find_nearest(dt,0.8) # time in seconds(s)
    end_idx   = find_nearest(dt,4.6) # time in seconds(s)
    alignment_subset = []
    for p in range(len(alignment_indices)):
        if start_idx < alignment_indices[p] < end_idx:
            alignment_subset.append(alignment_indices[p])
    # 
    print ('alignment subset',alignment_subset)
    alignment_indices = alignment_subset


    # This here is the problem, when there is a large DC offset it causes filter artefacts. 
    # Here I interpolate a line between the start and end of the data.     
    zeroed_fsignal = downsampled_data 
    for j in range(len(alignment_indices)):
        beginning_idx   = alignment_indices[j]-int(0.1*pulse_length*new_Fs)
        end_idx         = alignment_indices[j]+int(1.2*pulse_length*new_Fs)
        if end_idx >= len(zeroed_fsignal): # we have gone past the end of the file. 
            alignment_indices= alignment_indices[0:len(alignment_indices)-1]
            break
        # Define the known points
        x = [beginning_idx,end_idx]
        y = [zeroed_fsignal[beginning_idx], zeroed_fsignal[end_idx]]
        ...
        x_axis = np.linspace(beginning_idx,end_idx,(end_idx-beginning_idx) )
        y_new = polynomial(x_axis)
        zeroed_fsignal[beginning_idx:end_idx] =  y_new
    # 
    # d_df_data = zeroed_fsignal
    d_df_data = sosfiltfilt(sos_df_band, zeroed_fsignal)  
    ...
    # alignment_indices = alignment_indices[2:5]
    ...
    for i in range(len(alignment_indices)):  #
        ...
        if i % periods_of_interest == 0:
            ...
            print ('total height of segment:',height)
            if len(segment) == array_len: # ensure only full sections are appended.
                if (height) < 1000: # rudimentary filter. 
...
plot_filename = t_series+'_fswitch_events_'+str(n_events)+'.png'
