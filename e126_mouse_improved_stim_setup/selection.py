...
filename = 't3-pulse_anesthesia_data.npz'
outfile = 't3-pulse_anesthesia_data_selected.npz'
...
std_data = np.std(nfiltered_segmented_array,0)
...
for j in range(n_events-1):
    ...
    print ('std: ',np.std(portion_of_interest))

    timestep = 1/Fs
    N = len(portion_of_interest)
    fft_d = fft(portion_of_interest)
    ...
    try:
        ...
        print("Oops!  That was no valid number. Skipped that event. Try again...")
...
plot_filename = 't1_pulse_events_'+str(n_events)+'.png'
