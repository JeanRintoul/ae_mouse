...
file_multiplier = 2 
multifile_summation = [0]*file_multiplier*N
semisignal = [0]*tt[ss:ee]

print ('multifile ',len(multifile_summation))
...
for n in range(len(file_list)):
    ...
    # if n > 0:      # It is perfectly time synced. 
    #     fig = plt.figure(figsize=(6,6))
    #     ax  = fig.add_subplot(111)
    #     plt.plot(wholefile_summation,'k')
    #     plt.plot(fsignal,'g')
    #     plt.show()   
    #     
    if n == 0 :
        # semisignal = fsignal
        semisignal = sosfiltfilt(sos_demodulate_band, fsignal)
    elif (n+1)%file_multiplier == 0:
        #   
        # It shouldn't be a simple concatenate, ideally I'd do it so that I didn't have a disjunct area. 
        #    
        ffsignal = sosfiltfilt(sos_demodulate_band, fsignal)

        # seemlessly join two files together. 
        tseg               = np.linspace(0, len(ffsignal)*timestep, len(ffsignal), endpoint=False)
        gap = (1/carrier)/timestep
        # print ('gap:',gap)
        f1_peaks, properties    = find_peaks(semisignal,distance = gap )
        f2_peaks, properties    = find_peaks(ffsignal,distance = gap )
        # peaks, properties = find_peaks(rawdata_segment, height = (1000,10000) )
        print ('len peaks:',len(f1_peaks),f1_peaks[0:5])
        apodization_peak_avoidance = 100000
        # offset information of the second signal. 
        offset = f2_peaks[0] + f2_peaks[apodization_peak_avoidance]
        # this gives offset information for the first signal.  
        # final_peak = len(semisignal) - f1_peaks[-1] - apodization_peak_avoidance
        final_peak = len(semisignal)  - f1_peaks[len(f1_peaks)-apodization_peak_avoidance]        
        print ('offset index is:',offset,final_peak)
        # for the second file, adjust the beginning of the file. 
        second = [0]*len(ffsignal)
        if offset < 0:
            second[0: (len(second)+offset)] = ffsignal[-offset:]
        else:   # if the offset is positive,  
            second[0:(len(second)-offset)] = ffsignal[offset:]
        # for the first file, adjust the end of the file. 
        first = [0]*len(semisignal)
        if offset < 0:
            first[final_peak: ] = semisignal[0:(len(semisignal)-final_peak) ]
        else:   # if the offset is positive,  
            first[final_peak:] = semisignal[0:(len(semisignal)-final_peak)]
        # 
        double_signal       = np.concatenate((first,second))
        print ('len(first and second)',len(first),len(second))
        multifile_summation = multifile_summation + double_signal
        # 
        t2seg               = np.linspace(0, len(ffsignal)*2*timestep, 2*len(ffsignal), endpoint=False)
        halfway_pt = len(semisignal)*timestep
        #         
        fig = plt.figure(figsize=(6,6))
        ax  = fig.add_subplot(211)
        plt.plot(t2seg,multifile_summation,'k')
        plt.axvline(x = halfway_pt,color='r')
        # plt.plot(fsignal,'g')
        ax2  = fig.add_subplot(212)
        plt.plot(tseg,semisignal,'k')
        plt.plot(tseg[f1_peaks],semisignal[f1_peaks],'.b')
        plt.plot(tseg,fsignal,'r')   
        plt.plot(tseg[f2_peaks],ffsignal[f2_peaks],'.g')              
        plt.show()   
        #   
    else: 
        # semisignal = fsignal
        ...
        if i % periods_of_interest == 0:
            ...
            if len(rawdata_segment) == array_len*downsampling_factor:
                ...
                if n == 0:
                    ...
                    unaltared_rawdata_summation = rawdata_segment
                else: 
                    unaltared_rawdata_summation = unaltared_rawdata_summation + rawdata_segment
                    # Do time alignment of the 500kHz signal, so that both VEP and carrier have time alignment.  
                    tseg               = np.linspace(0, len(rawdata_segment)*timestep, len(rawdata_segment), endpoint=False)
                    gap = (1/carrier)/timestep
                    # print ('gap:',gap)
                    segment_peaks, properties   = find_peaks(rawdata_segment,distance = gap )
                    summation_peaks, properties = find_peaks(rawdata_summation,distance = gap )
                    # peaks, properties = find_peaks(rawdata_segment, height = (1000,10000) )
                    # print ('len peaks:',len(segment_peaks),segment_peaks[0:5])
                    offset = summation_peaks[0] - segment_peaks[0]
                    # print ('offset index is:',offset)
                    segment = [0]*len(rawdata_segment)
                    if offset < 0:
                        segment[0: (len(segment)+offset)] = rawdata_segment[-offset:]
                    else:   # if the offset is positive,  
                        segment[0:(len(segment)-offset)] = rawdata_segment[offset:]
                    # tiny offset correction, enabling time alignment at VEP time spans, as well as carrier time-syncing.    
                    # fig = plt.figure(figsize=(6,6))
                    # ax  = fig.add_subplot(111)
                    # plt.plot(tseg,rawdata_summation,'k')
                    # plt.plot(tseg[summation_peaks],rawdata_summation[summation_peaks],'.k')
                    # plt.plot(tseg,rawdata_segment,'r')
                    # plt.plot(tseg[segment_peaks],rawdata_segment[segment_peaks],'.r')
                    # plt.plot(tseg,segment,'g')
                    # # plt.plot(segment,'g')
                    # ax.set_xlim([0,0.0001])
                    # plt.show()
                    # if len(y) == len(rawdata_summation):
                    rawdata_summation = rawdata_summation + segment
                    #                     
                    #      
            # LFP data.
...
# 
# 
multifile               = multifile_summation/iteration
multifileN              = len(multifile)
multifile_window        = np.kaiser( (multifileN), 40 )
xf                      = np.fft.fftfreq( (multifileN), d=raw_timestep)[:(multifileN)//2]
raw_mfrequencies        = xf[1:(multifileN)//2]
# 
fft_mraw                = fft(multifile*multifile_window)
fft_mraw                = np.abs(2.0/(multifileN) * (fft_mraw))[1:(multifileN)//2]
# 
print ('multifile f bin size:',raw_mfrequencies[1]-raw_mfrequencies[0])
print ('wholefile f bin size:',raw_wfrequencies[1]-raw_wfrequencies[0])
...
unaltared_rawdata_summation = unaltared_rawdata_summation/segment_summation_iteration
s_a             = int(0)
...
fft_unaltared_raw  = fft(unaltared_rawdata_summation[s_a:e_a]*raw_window)
fft_unaltared_raw  = np.abs(2.0/(e_a-s_a) * (fft_unaltared_raw))[1:(e_a-s_a)//2]
print ('segment f bin size:',raw_frequencies[1]-raw_frequencies[0])
# 
...
w_start_idx = find_nearest(raw_wfrequencies,carrier+3 )   # after the first 5Hz. 
w_end_idx   = find_nearest(raw_wfrequencies,carrier+40)
...
# segmented
s_start_idx = find_nearest(raw_frequencies,carrier+3 )   # after the first 5Hz. 
s_end_idx   = find_nearest(raw_frequencies,carrier+40)
...
plt.plot(raw_mfrequencies,fft_mraw,'r')
...
plt.plot(raw_frequencies,fft_unaltared_raw,'r')
ax2.set_xlim([carrier-40,carrier+40])
ax2.set_ylim([0,1])
plt.legend(['segmented time synced','segmented not time synced'],loc='upper right')
