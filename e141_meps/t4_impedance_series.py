...
saveprefix  = './/images//'
# 
savepath             = 'D:\\ae_mouse\\e141_meps\\t4_phantom\\impedance\\'
...
for i in range(len(freqs)):
    for j in range(n_repeats):
        ...
        fft_v     = fft(vsignal[start_idx:end_idx])
        fft_v     = np.abs(2.0/(newN) * (fft_v))[1:(newN)//2]
        fft_i     = fft(isignal[start_idx:end_idx])
        fft_i     = np.abs(2.0/(newN) * (fft_i))[1:(newN)//2]

        current_idx      = find_nearest(frequencies,frequency)

        impedance = fft_v[current_idx]/fft_i[current_idx]
...
np.savez(outfile,fs=fs,impedances=impedances)
