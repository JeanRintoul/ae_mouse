...
saveprefix  = './/images//'
# 
savepath_noF21         = 'D:\\ae_mouse\\e145_meps\\t1_phantom\\'
savepath_F21           = 'D:\\ae_mouse\\e145_meps\\t2_mouse\\'

out = 'D:\\ae_mouse\\e145_meps\\images\\'
# 
# 
filename          = 'aemeps_pressure_ramp'
freqs            = [0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4]
f = 18 
# 
# 
print ('filename: ', filename)
data                 = np.load(savepath_noF21+filename+'.npz', mmap_mode='r')
fs                   = data['fs']
amp                  = data['amps']
dcs                  = data['dcs']
flist                = data['flist']
...
for i in range(len(freqs)):
	idx= np.where(aa == freqs[i])[0]
	mean_amp = np.mean(amp[idx])
	mean_dc = np.mean(dcs[idx])
	amp_means.append(mean_amp)
	dc_means.append(mean_dc)
x = fs*10 
y = amp
m, b = np.polyfit(x,y, 1)
# 
# 
data                 = np.load(savepath_F21+filename+'.npz', mmap_mode='r')
fs21                   = data['fs']
amp21                  = data['amps']
dcs21                  = data['dcs']
flist21                = data['flist']
frequencies21          = data['frequencies']
# 
aa21 = np.array(fs21)
amp_means21  = []
dc_means21   = []
for i in range(len(freqs)):
	idx= np.where(aa21 == freqs[i])[0]
	mean_amp = np.mean(amp21[idx])
	mean_dc = np.mean(dcs21[idx])
	amp_means21.append(mean_amp)
	dc_means21.append(mean_dc)
x21 = fs*10 
y21 = amp21
m21, b21 = np.polyfit(x21,y21, 1)
...
plt.plot(x, y, 'ro')
plt.plot(x21, y21, 'ko')
plt.legend(['no F21','F21'],loc='upper left',fontsize=f,framealpha=0 )
plt.plot( x, m*x+b, '--r')
plt.plot(x21, m21*x21+b21, '--k')
...
plt.tight_layout()
plot_filename = out+'_'+filename+'.png'
plt.savefig(plot_filename)
plt.show()

fig = plt.figure(figsize=(4,4))
ax  = fig.add_subplot(111)
plt.plot(fs*10,dcs,'ro')
plt.plot(fs21*10,dcs21,'ko')

plt.legend(['no F21','F21'],loc='upper left',fontsize=f,framealpha=0 )
plt.plot(np.array(freqs)*10,dc_means,'--r')
plt.plot(np.array(freqs)*10,dc_means21,'--k')
#
plt.xticks(fontsize=f)
plt.yticks(fontsize=f)
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
plt.title('DC offsets')
plt.tight_layout()
plot_filename = out+'_'+filename+'dcs.png'
plt.savefig(plot_filename)
plt.show()
...
# ax  = fig.add_subplot(111)
# plt.plot(frequencies,flist.T)
# plt.legend(freqs,loc='upper right')
...
# np.savez(outfile,flist=flist,fs=fs,amps=amps)
# print ('saved out a data file!')

# files = ['1hz_g500_300uv_pp_1h232.npy',
# '1hr33_whiskers.npy',
# '1hr35.npy',
# '1hr36_whiskers.npy',
# '1hz_1hr46.npy',
