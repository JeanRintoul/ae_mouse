...
for k in range(len(frequencies)):
	...
	for i in range(a): # iterate through each file.
		...
		if m_internal_list[1] == element: # frequency match. 
			...
			diff = (len(m_peaks_measured) - len(m_peaks_applied))
			if (len(m_peaks_measured) - len(m_peaks_applied)) > 0:
				m_peaks_measured = m_peaks_measured[diff:]
...
plt.plot(t,m_applied,alpha=0.3,color='k')
plt.plot(t,m_response)
plt.yticks(fontsize=16)
plt.xticks(fontsize=16)
ax2  = fig.add_subplot(212)
plt.plot(t,p_applied,alpha=0.3,color='k')
