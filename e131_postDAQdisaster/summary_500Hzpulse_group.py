...
# 
savepath    = 'D:\\ae_mouse\\e131_postDAQdisaster\\t3_mouse\\500Hz_ERPs_vchan\\'
gain        = 1
m_channel   = 0 
rf_channel  = 4
marker_channel = 7 
...
end                             = (periods_of_interest-1)*(1.0/prf)+0.5*(1.0/prf)
...
for n in range(len(file_list)):
    ...
    for i in range(len(alignment_indices)):  #
        ...
        if i % periods_of_interest == 0:
            ...
            if (np.max(segment)-np.min(segment)) < 100000000: # rudimentary filter. 
...
plot_filename = t_series+'_100Hzpulse_'+str(prf)+'events_'+str(n_events)+'.png'
