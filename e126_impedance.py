...
second = 1 
# second = carrier - 10 
dfx     = carrier - second  # This means we should get one dfx cycle in the pulse. 
...
'duration': duration, 
'position': test_no,
'pressure_amplitude': 1.0,      # how much is lost through skull??? 400kPa, 0.08 is about 200kPz. 0.15 is about 400kPa. 
...
'save_folder_path':'D:\\ae_mouse\\e126_mouse_improved_stim_setup',
...
for i in x:
    ...
    # lfp band filter. 
    df_l = carrier - 2*second 
    df_h = carrier + 2*second
    sos_df_band = iirfilter(17, [df_l, df_h], rs=60, btype='bandpass',
                           analog=False, ftype='cheby2', fs=Fs,
                           output='sos')
    # sos_c_band = iirfilter(17, [carrier-2*dfx, carrier+2*dfx], rs=60, btype='bandpass',
    #                        analog=False, ftype='cheby2', fs=Fs,
    #                        output='sos')
    # 
    ...
    isignal           = data[i_channel]/49.9 

    # c_data              = sosfiltfilt(sos_c_band, fsignal)
    dfx_data              = sosfiltfilt(sos_df_band, fsignal)
    ...
    fft_i = fft(isignal[start_pause:end_pause])
    ...
    # print( 'df,carrier,rfmonitor: ',2*fft_m[df_idx],2*fft_m[carrier_idx],2*fft_rf[carrier_idx])
    print ('impedance = v/i : ', 2*fft_v[carrier_idx],2*fft_i[carrier_idx])
    print ('impedance = ', fft_v[carrier_idx]/fft_i[carrier_idx])
