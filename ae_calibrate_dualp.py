...
acoustic_frequency2 = aeti_variables['pi_frequency']
...
df = dfx +broken_offset
sf = 2*acoustic_frequency + dfx + broken_offset
...
carrier2_idx = m.find_nearest(frequencies,acoustic_frequency2)
...
print ('Amplitude at c1 and c2:',2*fft_m[carrier_idx],2*fft_m[carrier2_idx])
# now = datetime.now()
...
plt.plot(frequencies,fft_us,'r')
# plt.plot(frequencies,fft_v,'r')
# plt.plot(frequencies,fft_m,'k')
plt.legend(['fft us'],loc='upper right')
ax.set_xlim([0,int(np.max(frequencies))/2 + dfx*3])

ax2 = fig.add_subplot(212)
plt.axvline(df,color='r')
plt.axvline(sf,color='r')
# plt.plot(frequencies,fft_us,'b')
# plt.plot(frequencies,1e6*fft_v,'r')
plt.plot(frequencies,fft_m,'k')
# plt.plot(frequencies,fft_i,'orange')
# ax2.set_ylim([0,500])
ax2.set_xlim([0,int(np.max(frequencies))/2 +dfx*3])
