...
int main(int argc, char* argv[])
{
  ...
  for(uint32_t index = 0; index < deviceCount; index++)
  {
    ...
    if(tiepie_hw_devicelistitem_can_open(item,TIEPIE_HW_DEVICETYPE_GENERATOR)&& productId==TIEPIE_HW_PRODUCTID_WS5)
    {
      gen_current =   tiepie_hw_devicelistitem_open_generator(item);
      printf("Generator current opened: %d\n",productId);
    }
    if(tiepie_hw_devicelistitem_can_open(item,TIEPIE_HW_DEVICETYPE_GENERATOR) && productId== TIEPIE_HW_PRODUCTID_HS5)
    {
  ...
  if(deviceHandleCount > 1)
  {
    ...
    scp = tiepie_hw_devicelist_create_and_open_combined_device(deviceHandles, deviceHandleCount);
    CHECK_LAST_STATUS();
  ...
  if(scp != TIEPIE_HW_HANDLE_INVALID)
  {
    ...
    printf("channel count of the scope: %d\n",channelCount);
  }
  else {
    status = EXIT_FAILURE;
    goto exit_no_mem;
