...
int main(int argc, char* argv[])
{
  ...
  for(uint32_t index = 0; index < tiepie_hw_devicelist_get_count(); index++)
  {
    ...
    if(tiepie_hw_devicelistitem_can_open(item, TIEPIE_HW_DEVICETYPE_GENERATOR))
    {
      ...
      if(gen != TIEPIE_HW_HANDLE_INVALID && (tiepie_hw_generator_get_modes_native(gen) & TIEPIE_HW_GM_GATED_PERIODS) && tiepie_hw_device_trigger_get_input_count(gen) > 0)
      {
  ...
  if(gen != TIEPIE_HW_HANDLE_INVALID)
  {
    ...
    // Set frequency:
    tiepie_hw_generator_set_frequency(gen, 10e3); // 10 kHz
    CHECK_LAST_STATUS();

    // Set amplitude:
    tiepie_hw_generator_set_amplitude(gen, 5); // 5 V
    CHECK_LAST_STATUS();
    ...
    // Set mode:
    tiepie_hw_generator_set_mode(gen, TIEPIE_HW_GM_GATED_PERIODS);
    CHECK_LAST_STATUS();

    // Locate trigger input:
    const uint16_t index = tiepie_hw_device_trigger_get_input_index_by_id(gen, TIEPIE_HW_TIID_EXT1);
    CHECK_LAST_STATUS();
  ...
  {
    fprintf(stderr, "No generator available with gated burst support!\n");
    status = EXIT_FAILURE;
  }
