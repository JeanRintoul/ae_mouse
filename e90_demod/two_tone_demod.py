...
# fs                    = 14 
fc                    = 500000
# file_number           = 37   # vep with pressure. 
# data_filepath         = 'D:\\mouse_aeti\\e86_demod\\t9_rf_noise_investigation\\'


file_number           = 7   # vep with pressure. 
...
start    = 500
stop     = 10000
sos_low  = iirfilter(27, [start,stop], rs=60, btype='bandpass',
...
# # plt.plot(t,rf_amplitudes/np.max(rf_amplitudes),'b')
# plt.plot(t,0.5*carrier_filtered_raw/np.max(carrier_filtered_raw) ,'g')
plt.plot(td,dfiltered_rawdata/np.max(dfiltered_rawdata),'k')
plt.plot(td,dfiltered_demoddata/np.max(dfiltered_demoddata),'r')
