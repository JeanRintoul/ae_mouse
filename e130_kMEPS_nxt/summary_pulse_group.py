...
#
# outfile = 't2-fswitch_anesthesia_data.npz'
t_series = 't1'
outfile = t_series + '-pulse_data.npz'
# 
savepath    = 'D:\\ae_mouse\\e130_kMEPS_nxt\\US_pulse_stimulation\\'
gain        = 500
m_channel   = 0 
rf_channel  = 4

pulse_length = 0.05 #  pulse length in seconds. 
...
for n in range(len(file_list)):
    ...
    alignment_indices = alignment_indices[::2]
    print ('the alignment_indices are:',alignment_indices)   
    ...
    # alignment_indices = alignment_indices[1:4]
    ...
    for i in range(len(alignment_indices)):  #
        ...
        if i % periods_of_interest == 0:
            ...
            if (np.max(segment)-np.min(segment)) < 100000: # rudimentary filter. 
...
plot_filename = t_series+'_pulse_'+str(prf)+'events_'+str(n_events)+'.png'
