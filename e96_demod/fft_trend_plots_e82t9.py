...
for i,j in zip(times,modulated_Thirty):
    ax.annotate('%s' %file_list[num], xy=(i,j)) 
    num = num + 1 
plt.legend(['modulated '+str(led_frequency)+'Hz','modulated '+str(2*led_frequency)+'Hz'])
ax.set_xlabel('Time(s)')
ax.set_ylabel('Volts ($\mu$V)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
plot_filename = filename + '_modulated_vs_time.png'
...
fig = plt.figure(figsize=(5,5))
ax = fig.add_subplot(111)
plt.plot(times,VEP,color='purple')
plt.plot(times,Thirty,color='magenta')
num = 0 
for i,j in zip(times,VEP):
    ax.annotate('%s' %file_list[num], xy=(i,j)) 
    num = num + 1 
num = 0 
for i,j in zip(times,Thirty):
    ax.annotate('%s' %file_list[num], xy=(i,j)) 
    num = num + 1 
plt.legend(['VEP'+str(led_frequency)+'Hz','2nd harmonic '+str(2*led_frequency)+'Hz'])
ax.set_xlabel('Time(s)')
ax.set_ylabel('Volts ($\mu$V)')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
plot_filename = filename + '_VEPs_vs_time.png'
plt.savefig(plot_filename)
plt.show()
