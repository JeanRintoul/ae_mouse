...
# 
# Test 1: 8 hz files. 
frequencies_8               = [8,16,32]
frequencies_of_interest     = [carrier-frequencies_8[0],carrier+frequencies_8[0],carrier+frequencies_8[1],carrier-frequencies_8[1],carrier+frequencies_8[2],carrier-frequencies_8[2]]
# eight_filelist              = ['t1_8Hz','t2_8Hz','t3_8Hz','t5_8Hz','t6_8Hz']
# ten_filelist                = ['t1_10Hz','t2_10Hz','t3_10Hz','t5_10Hz','t6_10Hz']

# Test 2: 
# 8Hz and 10hz frequencies of interest. 
frequencies_10            = [10,20,30]
frequencies_of_interest   = [carrier-frequencies_10[0],carrier+frequencies_10[0],carrier+frequencies_10[1],carrier-frequencies_10[1],carrier+frequencies_10[2],carrier-frequencies_10[2]]

# eight_filelist            = ['139t7_8Hz','139t5_8Hz','t1_8Hz','t2_8Hz','t5_8Hz']
# ten_filelist              = ['t8_10Hz','139t5_10Hz','t1_10Hz','t2_10Hz','t5_10Hz']
# 
# 
eight_filelist            = ['t1_8Hz','139t5_8Hz','t1_8Hz','t2_8Hz','t5_8Hz']
ten_filelist              = ['t8_10Hz','139t5_10Hz','t1_10Hz','t2_10Hz','t5_10Hz']
...
for i in range(len(ten_filelist)):
  ...
  filename_eight  = eight_filelist[i]+'.npz'
  # 
  print ('10hz filename: ', filename_ten)
  data                 = np.load(savepath+filename_ten, mmap_mode='r')
  t                    = data['t']
  ten                = data['rawdata_summation']
  print ('8hz filename: ', filename_eight)
  datast               = np.load(savepath+filename_eight, mmap_mode='r')
  eight                 = datast['rawdata_summation']
  ...
  # #
  fft_eight         = fft(eight*window)
  fft_eight         = np.abs(2.0/(N) * (fft_eight))[1:(N)//2]
  ...
  for n in range(len(frequencies_of_interest)): # sum all frequencies per unit time.
    ...
    kaiser_eight_totals.append(fft_eight[df_idx])
  ...
  dis_kaiser_eight_totals    = []
  dis_kaiser_ten_totals      = []
  for n in range(start_idx, end_idx): # sum all frequencies per unit time.
    if n not in interest_frequencies: 
      dis_kaiser_eight_totals.append(fft_eight[n])
      dis_kaiser_ten_totals.append(fft_ten[n]) 
  kaiser_eight_snr    = 20*np.log(np.mean(kaiser_eight_totals)/np.mean(dis_kaiser_eight_totals))
  kaiser_ten_snr      = 20*np.log(np.mean(kaiser_ten_totals)/np.mean(dis_kaiser_ten_totals))
  print ('kaiser eight/ten',np.round(kaiser_eight_snr,2),np.round(kaiser_ten_snr,2) )

  ten_snrs.append(kaiser_ten_snr)
  eight_snrs.append(kaiser_eight_snr)
# 
# 
mean_ten    = np.mean(ten_snrs)
mean_eight  = np.mean(eight_snrs)
std_ten     = np.std(ten_snrs)
std_eight   = np.std(eight_snrs)


print ('means and stds:',mean_eight,mean_ten,std_ten,std_eight)
...
materials = ['8Hz VEP', '10Hz VEP']
x_pos     = np.arange(len(materials))
CTEs      = [mean_eight,mean_ten]
error     = [std_ten,std_eight]
