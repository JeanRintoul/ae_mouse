...
frequencies_of_interest     = [2,4,6,8]   
...
for n in range(len(file_list)):
    ...
    sos_demodulate_bandpass  = iirfilter(17, [carrier_x - bandwidth_of_interest,carrier_x+bandwidth_of_interest], rs=60, btype='bandpass',
                           analog=False, ftype='cheby2', fs=new_Fs,
                           output='sos')
    ...
    # IQ demodulate.  
    print ('carrier',carrier_x)
    demodulated_signal = -demodulate(filtered_downsampled_data,carrier_x,dt)
    ...
    for j in range(len(f_band_test)):
        ...
        # now it overlaps 
        # carrier = f_band_test[j]
        # df_cut  = carrier - bandwidth_of_interest
        # df_high = carrier + bandwidth_of_interest
        # print ('df cut and high',df_cut,df_high,carrier)
    ...
    # normalize each result that is the best.     
    averaged_demodulated = biggest/np.max(biggest)
    ...
    downsampled_raw_lfp.append(downsampled_data)
...
downsampled_raw_lfp_array = np.array(downsampled_raw_lfp)
# print ('shape',demod_segmented_array.shape)
# # 
average_raw  = np.mean(downsampled_raw_lfp_array,axis=0)
...
fft_raw_spectrum = fft(average_raw[start_index:end_index])
fft_raw = np.abs(2.0/(newN) * (fft_raw_spectrum))[1:(newN)//2]
...
plt.yticks(fontsize=16)
plt.xticks(fontsize=16)
plt.legend(['IQ FFT'],loc='upper right',frameon=False,framealpha=1.0, fontsize=16)
ax3.spines['right'].set_visible(False)
ax3.spines['top'].set_visible(False)

plt.tight_layout()
plot_filename = saveprefix+'FFT_ALL.png'
plt.savefig(plot_filename, transparent=True)
plt.show()

fig = plt.figure(figsize=(3,3))
ax  = fig.add_subplot(111)
plt.plot(frequencies,fft_raw,color='k')
ax.set_xlim([carrier_x-bandwidth_of_interest,carrier_x+bandwidth_of_interest])
ax.set_ylabel('Volts ($\mu$V)',fontsize=16)
ax.set_xlabel('Frequency(Hz)',fontsize=16)
plt.yticks(fontsize=16)
plt.xticks(fontsize=16)
...
ax.spines['top'].set_visible(False)
plt.tight_layout()
plot_filename = saveprefix+'FFT_Carrier.png'
plt.savefig(plot_filename, transparent=True)
plt.show()
...
ax.spines['top'].set_visible(False)
plt.tight_layout()
plot_filename = saveprefix+'FFT_BESTOF_DM.png'
plt.savefig(plot_filename, transparent=True)
plt.show()
