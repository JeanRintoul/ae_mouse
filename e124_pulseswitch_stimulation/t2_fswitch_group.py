...
#
outfile = 'fswitch_anesthesia_data.npz'
# outfile = 'fswitch_data.npz'

savepath    = 'D:\\ae_mouse\\e124_pulseswitch_stimulation\\t2\\'
gain        = 500
m_channel   = 0 
rf_channel  = 2
...
start                           = 0.4*(1.0/prf)   # 0.2 seconds
end                             = (periods_of_interest-1)*(1.0/prf)+0.6*(1.0/prf)
...
for n in range(len(file_list)):
    ...
    # 
    sos_df_band = iirfilter(17, [0.5,df_cut], rs=60, btype='bandpass',
                           analog=False, ftype='cheby2', fs=new_Fs,
                           output='sos')
    # 
    # 
    sos_downsampling_band = iirfilter(17, [1,new_Fs/2], rs=60, btype='bandpass',
                           analog=False, ftype='cheby2', fs=Fs,
                           output='sos')
    # 
    # #
    ...
    for i in range(len(mains_list)):
        sos_mains_stop = iirfilter(17, [mains_list[i]-2 , mains_list[i]+2 ], rs=60, btype='bandstop',
                               analog=False, ftype='cheby2', fs=new_Fs,
                               output='sos')
        downsampled_data = sosfiltfilt(sos_mains_stop , downsampled_data)
...
plot_filename = 't1_fswitch_prf_'+str(prf)+'events_'+str(n_events)+'.png'
