...
df = int(1000)
sf = int(abs(acoustic_frequency*2+1000))
# 
...
sos_lowdf = iirfilter(17, [low,high], rs=60, btype='bandpass',
                       ...
                       output='sos')
dfsignal  = sosfiltfilt(sos_lowdf, 1e6*data[m_channel]/gain )
# 
# 
sf = 500000*2+1020
low  = sf - 4
if low < 0:
    low = 0.1
high = sf+4
sos_lowsf = iirfilter(17, [low,high], rs=60, btype='bandpass',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
sfsignal  = sosfiltfilt(sos_lowsf, 1e6*data[m_channel]/gain )
...
if low < 0:
    low = 0.1
high = carrier+4
sos_lowc = iirfilter(17, [low,high], rs=60, btype='bandpass',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
csignal  = sosfiltfilt(sos_lowc, 1e6*data[m_channel]/gain )
...
plt.plot(t,sfsignal,'r')
ax  = fig.add_subplot(313)
plt.plot(t,csignal,'b')
plt.show()
# 
# 
