...
def demodulate(measured_signal):
    ...
    offset_adjustment   = offset*np.cos(2 * np.pi * pressure_signal_frequency * t)
    IQ                  = (measured_signal+offset_adjustment)*np.exp(1j*(2*np.pi*pressure_signal_frequency*t )) 
    idown               = np.real(IQ)
    qdown               = np.imag(IQ)
...
carrier_f               = pressure_signal_frequency
filter_cutoff           = 200
