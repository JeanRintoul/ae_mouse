...
int main(int argc, char* argv[])
{
  ...
  double chEnables[6]                 = {1,1,1,1,1,1};
  ...
  const uint16_t trigger_output = DevTrGetOutputIndexById(gen_current,TIID_EXT3);
  //Enable the trigger output. 
  DevTrOutSetEnabled(gen_current, trigger_output, BOOL8_TRUE);
  // Set the event type to manual. Device, Output, Event.
  DevTrOutSetEvent(gen_current,trigger_output,TOE_MANUAL);
  // Output invert. 
...
// Define the constants used. 
  ...
  double dRanges[6] = {20.0,20.0,80.0,4.0,40.0,40.0}; // ch 6 was 40.0   
  // create array of AC or DC coupling. 
  uint64_t Couplings[6] = {CK_DCV,CK_DCV,CK_DCV,CK_DCV,CK_DCV,CK_DCV};
  // 
  ...
  active_channel_count= chEnables[0] + chEnables[1] + chEnables[2] + chEnables[3] + chEnables[4] + chEnables[5];
...
// Setup events:
    ...
    // DevTrOutTrigger(gen_pressure, trigger_output);
    // CHECK_LAST_STATUS();  
    DevTrOutTrigger(gen_current, trigger_output);
    CHECK_LAST_STATUS();  
