...
int main(int argc, char* argv[])
{
  ...
  double sample_frequency   = 1e7;      // 100Mhz sample rate 
  ...
  double current_amplitude  = 12;
  bool raw = false;
  // Create array of data ranges. 
  // RF ampx10, hydrophone,  e output, current out, vmon x10, 0, aefield, 0
  double dRanges[8] = {2.0,2.0,12.0,4.0,20.0,2.0,2.0,2.0};
  ...
  // // Set frequency:
  // GenSetFrequency(gen_current, 1e3); // 1 kHz
  // CHECK_LAST_STATUS();
  ...
  // f is the frequency of the sine wave I wish to determine the load information for. 
  double f = 8000; 
  // float gain = 0.5;
  // sin(2*pi*f/fs)
  ...
  else if(ScpIsDataReady(scp))
  {
    ...
    // Write .npy file
    const char* filename = "load_block.npy";
