...
int main(int argc, char* argv[])
{
  ...
  double gen_pressure_sample_frequency  = 1e5;    // 5Mhz sample rate 
  double gen_current_sample_frequency   = 1e5;    // 5Mhz sample rate 
  ...
  double start_pause = 0.1;
  double end_pause = 0.9;  
  double ti_frequency = 0.0;    
  while ((opt = getopt(argc, argv, "f:g:p:d:s:a:b:t:e:z:m:")) != -1) {    
      switch(opt) {
      case 'f':
...
// Define the constants used. 
  ...
  double dRanges[8] = {20.0,80.0,20.0,0.2,8.0,2.0,2.0,4.0};  
  ...
  {  
    ...
    else if (i <= pressure_final_end_ramp) {  // the main pressure signal 
    ...
    else // the end zero point of pressure. Alternatively this could be a ramp. 
    {
      datap[i] = 0.0;
    }
  }
  ...
  {  
    ...
    {  
      // if (i<=current_pulse_length*3) {  // this should be in wavelengths
      //   data[i] = 0.05; // this will be a 20th of the the maximum. 
      // }      
      // else if (i>current_pulse_length*3 && i <=current_pulse_length*6 ) {
      //   data[i] = 0.0; // this will be a 20th of the the maximum.
      // }
      // else if (i>current_pulse_length*6 && i <=current_pulse_length*12 ) {
      //   data[i] = 0.2; // this will be a 20th of the the maximum.
      // } 
      // else if (i>current_pulse_length*12 && i <=current_pulse_length*15 ) {
      //   data[i] = 0.1; // this will be a 20th of the the maximum.
      // }       
      ...
      if (i<=current_pulse_length*3) {  // this should be in wavelengths
        ...
        data[i] = 0.4; // this will be a 20th of the the maximum.
      } 
      else if (i>current_pulse_length*12 && i <=current_pulse_length*15 ) {
        data[i] = 0.2; // this will be a 20th of the the maximum.
      }      
        ...
        if (no_ramp == 0.0) {         // 
          ...
          if (ti_frequency > 0.0) {  // add two sine waves together.
            data[i] = current_factor*(sinf(2*PI* current_signal_frequency * (float)i/gen_current_sample_frequency)+sinf(2*PI* ti_frequency * (float)i/gen_current_sample_frequency) +dc_offset);  
          }
          else {
    ...
    //  
      ...
      if (ti_frequency > 0.0) {  // add two sine waves together. 
        data[i]  = sinf(2*PI* current_signal_frequency * (float)i/gen_current_sample_frequency) +sinf(2*PI* ti_frequency * (float)i/gen_current_sample_frequency) +dc_offset;
      }
    ...
    else {  // assigns data into the ramp. 
      if (no_ramp == 0.0) {  // 
          current_factorend = (float)(current_duration_in_samples - i)/(float)current_end_ramp;
          if (ti_frequency > 0.0) {  // add two sine waves together. 
            data[i] = current_factorend*(sinf(2*PI* current_signal_frequency * (float)i/gen_current_sample_frequency)+sinf(2*PI* ti_frequency * (float)i/gen_current_sample_frequency) +dc_offset);        
          }
          else {  // don't add two sine waves together.
            current_factorend = (float)(current_duration_in_samples - i)/(float)current_end_ramp;
  ...
  // This ensures there is still a marker, if the current amplitude is 0. 
