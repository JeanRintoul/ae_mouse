/**
 * simth implements a timed generator beginning of a longer streamed measurement. 
 *
...
int main(int argc, char* argv[])
{
  ...
  // double sample_frequency             = 6e6;    // 5Mhz sample rate 
  // double duration                     = 0.1;    // measured in seconds
  ...
  double current_amplitude            = 1.0; //0.5616; 
  double pressure_amplitude           = 0.0;
  double current_signal_frequency     = 100000;
  // double pressure_signal_frequency    = 100000;  
  ...
  // double dRanges[8] = {20.0,0.2,20.0,20.0,8.0,8.0,20.0,20.0};
  double dRanges[8] = {20.0,0.2,20.0,4.0,8.0,8.0,20.0,20.0};  
  ...
  double pulse_length = sample_frequency/current_signal_frequency;

  int start_time = 0.0; //0.01*duration_in_samples;  // start index  // if total length is 0.1 seconds. and lag thing is 0.07
  int end_time   = pulse_length*30; //0.1*duration_in_samples;  // end index
  //   
  ...
  {  
    ...
    {
      datap[i] = sinf(2*PI* current_signal_frequency * (float)i/sample_frequency);
