...
#define DATA_READY \
  { \
    uint64_t samplesRead =  tiepie_hw_oscilloscope_get_data(scp, channelData, channel_count, 0, record_length); \
    for (int j = 0; j < channel_count; j++)  \
      fwrite(channelData[j], sizeof(float), record_length, f);  \
    currentSample += samplesRead;  \
    if(tiepie_hw_get_last_status() < TIEPIE_HW_STATUS_SUCCESS) \
    { \
      fprintf(stderr, "%s failed: %s\n", raw ? "ScpGetDataRaw" : "ScpGetData", tiepie_hw_get_last_status_str()); \
      status = EXIT_FAILURE; \
      goto exit; \
    } \
    n++; \
...
int main(int argc, char* argv[])
{
  ...
  tiepie_hw_handle gen_current  = TIEPIE_HW_HANDLE_INVALID;
  tiepie_hw_handle gen_pressure = TIEPIE_HW_HANDLE_INVALID;
  ...
  for(uint32_t index = 0; index < tiepie_hw_devicelist_get_count(); index++)
  {
    ...
    uint32_t product_id = tiepie_hw_devicelistitem_get_product_id(item);
    ...
    if(tiepie_hw_devicelistitem_can_open(item,TIEPIE_HW_DEVICETYPE_GENERATOR)&& product_id==TIEPIE_HW_PRODUCTID_WS5)
    {


      gen_current =   tiepie_hw_devicelistitem_open_generator(item);
      printf("Generator current opened: %d\n",product_id);
    }
    if(tiepie_hw_devicelistitem_can_open(item,TIEPIE_HW_DEVICETYPE_GENERATOR) && product_id== TIEPIE_HW_PRODUCTID_HS5)
    {
      gen_pressure =  tiepie_hw_devicelistitem_open_generator(item);
  ...
  // uint32_t gc_serialNumber = tiepie_hw_device_get_serial_number(gen_current);
  // uint32_t gp_serialNumber = tiepie_hw_device_get_serial_number(gen_pressure);    
  ...
  const uint16_t trigger_output = tiepie_hw_device_trigger_get_output_index_by_id(gen_pressure,TIEPIE_HW_TIID_EXT3);
  // Enable the trigger output. 
  tiepie_hw_device_trigger_output_set_enabled(gen_pressure, trigger_output, TIEPIE_HW_BOOL_TRUE);
  // Set the event type to manual. Device, Output, Event.
  tiepie_hw_device_trigger_output_set_event(gen_pressure,trigger_output,TIEPIE_HW_TOE_MANUAL);
  // Output invert. 
  tiepie_hw_generator_set_output_invert(gen_pressure, TIEPIE_HW_BOOL_TRUE);

  // try to set the output of the WS6DIFF to start when streaming starts. 
  const uint16_t trig_out_count = tiepie_hw_device_trigger_get_output_count(scp);
  printf("trigger output count: %d\n",trig_out_count); // 8!
  const uint16_t external_out = tiepie_hw_device_trigger_get_output_index_by_id( scp, ( DN_SUB_THIRD << TIEPIE_HW_TIOID_SHIFT_DN ) |TIEPIE_HW_TOID_EXT1 );
  printf("external out: %d\n",external_out); // 8!  
  // Enable the trigger output. 
  tiepie_hw_device_trigger_output_set_enabled(scp, external_out, TIEPIE_HW_BOOL_TRUE);
  // Set the event type to manual. Device, Output, Event.
  tiepie_hw_device_trigger_output_set_event(scp,external_out,TIEPIE_HW_TOE_OSCILLOSCOPE_RUNNING);

  // Set the trigger inputs
  const uint16_t input_pressure = tiepie_hw_device_trigger_get_input_index_by_id(gen_pressure, TIEPIE_HW_TIID_EXT3);
  // Enable trigger input:
  tiepie_hw_device_trigger_input_set_enabled(gen_pressure, input_pressure, TIEPIE_HW_BOOL_TRUE);
  CHECK_LAST_STATUS();
  tiepie_hw_device_trigger_input_set_kind(gen_pressure, input_pressure, TIEPIE_HW_TK_FALLINGEDGE);
  CHECK_LAST_STATUS();

  const uint16_t input_current =  tiepie_hw_device_trigger_get_input_index_by_id(gen_current, TIEPIE_HW_TIID_EXT3);
  // // Enable trigger input:
  tiepie_hw_device_trigger_input_set_enabled(gen_current, input_current, TIEPIE_HW_BOOL_TRUE);
  CHECK_LAST_STATUS();
  tiepie_hw_device_trigger_input_set_kind(gen_current, input_current, TIEPIE_HW_TK_FALLINGEDGE);
  CHECK_LAST_STATUS();  
// 
// Define the constants used. 
  ...
  double dRanges[8] = {4.0,40.0,20.0,20.0,80.0,4.0,4.0,40.0}; // ch 6 was 40.0  

  // create array of AC or DC coupling. 
  uint64_t Couplings[8] = {TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV,TIEPIE_HW_CK_DCV};
  // 
  const uint16_t channel_count = tiepie_hw_devicelistitemcombined_get_oscilloscope_channel_count(scp,serialNumber);
  ...
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 0, 50000000);   
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 1, 50000000); 
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 2, 50000000);
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 3, 50000000);
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 4, 50000000);
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 5, 50000000);
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 6, 50000000);
  tiepie_hw_oscilloscope_channel_set_bandwidth(scp, 7, 50000000);
  // 
  // 
  ...
  // Set signal type:
  tiepie_hw_generator_set_signal_type(gen_current, TIEPIE_HW_ST_ARBITRARY);
  CHECK_LAST_STATUS();
  tiepie_hw_generator_set_frequency_mode(gen_current,TIEPIE_HW_FM_SIGNALFREQUENCY);
  tiepie_hw_generator_set_frequency(gen_current, gen_current_sample_frequency); // 1 kHz
  CHECK_LAST_STATUS();
  ...
  if (long_recording > 0) { // long recording in seconds. 
    ...
    tiepie_hw_generator_set_mode(gen_current, TIEPIE_HW_GM_BURST_COUNT); 
    tiepie_hw_generator_set_burst_count(gen_current, 1 );
    // set the pressure mode to burst
    tiepie_hw_generator_set_mode(gen_pressure, TIEPIE_HW_GM_BURST_COUNT);
    tiepie_hw_generator_set_burst_count(gen_pressure, 1 );
  }
  ...
  tiepie_hw_generator_set_data(gen_current, NULL, 0);
  CHECK_LAST_STATUS();
  // GenSetData(gen_pressure, NULL, 0);
  // CHECK_LAST_STATUS();
  if ( tiepie_hw_generator_is_controllable(gen_current)) {
    uint64_t dlength = tiepie_hw_generator_get_data_length(gen_current);
    CHECK_LAST_STATUS();
    printf("data in generator: %llu\n",dlength );
  }
  else {
  ...
  // Set amplitude:
  tiepie_hw_generator_set_amplitude(gen_current, current_amplitude); // 2 V
  CHECK_LAST_STATUS();
  // Set offset:
  tiepie_hw_generator_set_offset(gen_current, 0); // 0 V
  CHECK_LAST_STATUS();
  ...
  tiepie_hw_generator_set_data(gen_current, data, current_duration_in_samples);
  CHECK_LAST_STATUS();
  ...
  // Enable output:
  tiepie_hw_generator_set_output_enable(gen_current, TIEPIE_HW_BOOL_TRUE);
  CHECK_LAST_STATUS();

  tiepie_hw_generator_set_signal_type(gen_pressure, TIEPIE_HW_ST_ARBITRARY);
  CHECK_LAST_STATUS();
  tiepie_hw_generator_set_frequency_mode(gen_pressure,TIEPIE_HW_FM_SIGNALFREQUENCY );
  tiepie_hw_generator_set_frequency(gen_pressure, gen_pressure_sample_frequency); // 1 kHz
  CHECK_LAST_STATUS();

  // Set amplitude:
  tiepie_hw_generator_set_amplitude(gen_pressure, pressure_amplitude); // 2 V
  CHECK_LAST_STATUS();

  // Set offset:
  tiepie_hw_generator_set_offset(gen_pressure, 0); // 0 V
  CHECK_LAST_STATUS();

  // Enable output:
  tiepie_hw_generator_set_output_enable(gen_pressure, TIEPIE_HW_BOOL_TRUE);
  CHECK_LAST_STATUS();
  // remove all data stored in generator. 
  tiepie_hw_generator_set_data(gen_pressure, NULL, 0);
  CHECK_LAST_STATUS();
  // Load the signal array into the generator:
  tiepie_hw_generator_set_data(gen_pressure, datap, pressure_duration_in_samples);
  CHECK_LAST_STATUS();

  // Start signal generation:
  tiepie_hw_generator_start(gen_current);
  CHECK_LAST_STATUS();

  // printf("last status: %d\n",CHECK_LAST_STATUS());
  tiepie_hw_generator_start(gen_pressure);
  CHECK_LAST_STATUS();
  ...
  // Set measure mode:
  // ScpSetMeasureMode(scp, TIEPIE_HW_MM_STREAM);
  ...
  {
      printf("  Ch%u: %s\n", i + 1, tiepie_hw_oscilloscope_channel_set_enabled(scp, i, chEnables[i] > 0 ? TIEPIE_HW_BOOL_TRUE : TIEPIE_HW_BOOL_FALSE) ? "enabled" : "disabled");
  }

  if(resolution != 0)
  {
    tiepie_hw_oscilloscope_set_resolution(scp, resolution);
  }

  sample_frequency = tiepie_hw_oscilloscope_set_sample_rate(scp, sample_frequency);
  printf("Sample frequency: %f MHz\n", sample_frequency / 1e6);

  printf("Resolution: %u bit\n", tiepie_hw_oscilloscope_get_resolution(scp));

  record_length = tiepie_hw_oscilloscope_set_record_length(scp, record_length);
  printf("Record length: %" PRIu64 " Samples\n", record_length);

  printf("Data rate: %f MB/s\n", (active_channel_count * ceil(tiepie_hw_oscilloscope_get_resolution(scp) / 8.0) * sample_frequency) / 1e6);
  ...
  for(uint16_t ch = 0; ch < (channel_count); ch++)
  {
    ...
    {
      // Set coupling:
      tiepie_hw_oscilloscope_channel_set_coupling(scp, ch, Couplings[ch]); // DC Volt
      CHECK_LAST_STATUS();
      // Set range:
      tiepie_hw_oscilloscope_channel_set_range(scp, ch, dRanges[ch]); // 8 V
      CHECK_LAST_STATUS();

      double range =  tiepie_hw_oscilloscope_channel_get_range(scp,ch);
      if (range != dRanges[ch])
      {
...
// Setup events:
    ...
    tiepie_hw_object_set_event_callback(scp, events[0],NULL);
    tiepie_hw_object_set_event_callback(scp, events[1],NULL);
    tiepie_hw_object_set_event_callback(scp, events[2],NULL);
#else
    ...
    tiepie_hw_object_set_event_callbacky(scp, fd_data_ready);
    tiepie_hw_object_set_event_callback(scp, fd_data_overflow);
    ...
    tiepie_hw_device_trigger_output_trigger(gen_pressure, trigger_output);
    // CHECK_LAST_STATUS();  
    ...
    if(!tiepie_hw_oscilloscope_start(scp))
    {
    ...
    // Stop generator:
    tiepie_hw_generator_stop(gen_current);
    CHECK_LAST_STATUS();
    tiepie_hw_generator_stop(gen_pressure);
    CHECK_LAST_STATUS();
    ...
    // Disable output:
    tiepie_hw_generator_set_output_enable(gen_current, TIEPIE_HW_BOOL_FALSE);
    CHECK_LAST_STATUS();
    tiepie_hw_generator_set_output_enable(gen_pressure, TIEPIE_HW_BOOL_FALSE);
    CHECK_LAST_STATUS();
    ...
    // Close generator:
    tiepie_hw_object_close(gen_current);
    CHECK_LAST_STATUS();

    // Close generator:
    tiepie_hw_object_close(gen_pressure);
    CHECK_LAST_STATUS();
