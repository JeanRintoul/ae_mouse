...
// Define the constants used. 
  ...
  double dRanges[8] = {0.8,12.0}; // ch 6 was 40.0   

  // create array of AC or DC coupling. 
  uint64_t Couplings[2] = {CK_DCV,CK_DCV};
  // 
  const uint16_t channel_count = ScpGetChannelCount(scp);
  // 
  // Set the scope bandwidth, to remove high frequency noise.
  ScpChSetBandwidth(gen_pressure, 0, 50000000);   
  ScpChSetBandwidth(gen_pressure, 1, 50000000); 
  ...
  active_channel_count= chEnables[0] + chEnables[1];
