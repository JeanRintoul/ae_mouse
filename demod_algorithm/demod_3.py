...
def demodulate(measured_signal):
    ...
    phase = np.arctan2(qdown,idown)    # instantaneous phase
    # 
    ...
    # 
    # rsignal = mag[1:]*np.cos(inst_freq*t[1:])
    # rsignal = inst_freq
    # tf *= np.exp(-1j * delay * frequencies * 2 * np.pi)
    # rsignal *= np.exp(1j * inst_freq / 180.0 * np.pi)
    # rsignal = np.real(np.exp(1j*inst_freq*t[1:]))
    rsignal = -(idown + qdown)
...
demodulated_signal2,phase,regenerated_carrier,inst_freq,rsignal2 = demodulate2(input_signal )
...
rsignal2             = rsignal2[idx_start:idx_end]
...
plt.plot(t_cut,rsignal2,'purple')
plt.plot(t_cut,demodulated_signal,'g')
