...
# 
t_series        = 't2'
outfile         = t_series + '-mouse_data.npz'
Fs              = 5e6
new_Fs          = 1e4
...
sos_low = iirfilter(17, [bandwidth_of_interest], rs=60, btype='lowpass',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
...
rf_summation        = [0]*t
giant_data_array    = np.zeros( (len(file_list),len(t)) )
giant_marker_array  = np.zeros( (len(file_list),len(t)) )
for n in range(len(file_list)):
    ...
    # 
    lowf_signal         = sosfiltfilt(sos_low, fsignal) 
    # stack all the raw data for later processing, so I can get mean and std data. 
    giant_data_array[n,:]   = fsignal
    giant_marker_array[n,:] = marker  
    # 
    # 
    # 
...
np.savez(outfile,giant_marker_array=giant_marker_array,giant_data_array=giant_data_array ,averaged_marker=averaged_marker,averaged_raw_data=averaged_raw_data,n=len(file_list) )
...
window      = np.kaiser( (end_index-start_index), 14.0 )
