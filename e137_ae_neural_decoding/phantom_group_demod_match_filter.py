...
for n in range(len(file_list)):
    ...
    for i in range(len(template)):
        template[i] = np.cos( 2*np.pi*(frequency)*i*timestep) 
    # 
    ...
    # ax2.set_ylim([-1,1])
    # plt.legend(['LFP','Match Filter LFP','match demod'],loc='upper right')
    ...
    alignment_indices = alignment_indices[4:len(alignment_indices)-4]
...
# 
fft_davlfp   = fft(demod_average_lfp[start_pause:end_pause])
fft_davlfp   = np.abs(2.0/(end_pause-start_pause) * (fft_davlfp))[1:(end_pause-start_pause)//2]
# 
fft_mdavlfp   = fft(match_average_demod[start_pause:end_pause])
...
start_idx = find_nearest(frequencies,4 )   # after the first 5Hz. 
