...
# 
# Mouse tests, no mesh, instead I just used a wire... 2v out. 
# file:  it is gonna be too small   /   nothing much     /   12    /    
# f: 	5   /  10    /  30   / 100
# a: 	790 /  600   / 562   / 578  in fft 
# 
# So the way I saw the signal previously at low frquencies, was by making it BIG. 
# However, I currently have a charge injection limit and non-linearity problem to overcome. 
# 
# file_number 	= 15 # you can see spikes at 4Hz, but not in the FFT. 
# 
# file_number 	= 8
...
# 
# 
fstart_pause     = int(0.25 * N+1)
fend_pause       = int(0.5 * N-1)
sstart_pause     = int(0.5 * N+1)
send_pause       = int(0.75 * N-1)
theone = vsignal 
fft_first_data        = fft(theone[fstart_pause:fend_pause])
fft_first_data        = np.abs(2.0/(fend_pause-fstart_pause) * (fft_first_data))[1:(fend_pause-fstart_pause)//2]
fft_second_data        = fft(theone[sstart_pause:send_pause])
fft_second_data        = np.abs(2.0/(send_pause-sstart_pause) * (fft_second_data))[1:(send_pause-sstart_pause)//2]
xf              = np.fft.fftfreq( (fend_pause-fstart_pause), d=timestep)[:(fend_pause-fstart_pause)//2]
ffrequencies     = xf[1:(fend_pause-fstart_pause)//2]
xf              = np.fft.fftfreq( (send_pause-sstart_pause), d=timestep)[:(send_pause-sstart_pause)//2]
sfrequencies     = xf[1:(send_pause-sstart_pause)//2]
# 
...
df_l = 45
df_h = 55
sos_mains_stop = iirfilter(17, [df_l, df_h], rs=60, btype='bandstop',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')

df_l = 95
df_h = 105
sos_mains_stop2 = iirfilter(17, [df_l, df_h], rs=60, btype='bandstop',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
...
df_data     	= sosfiltfilt(sos_mains_stop, df_data)
df_data     	= sosfiltfilt(sos_mains_stop2, df_data)

spike_data      = sosfiltfilt(sos_spike_band, fsignal)
#
#
envelope_signal   = np.abs(hilbert(spike_data))
sos_envelope_lowp = iirfilter(17, [200], rs=60, btype='lowpass',
                       analog=False, ftype='cheby2', fs=Fs,
                       output='sos')
envelope_signal_lp   = sosfiltfilt(sos_envelope_lowp, envelope_signal)


fft_edata         = fft(envelope_signal_lp[start_pause:end_pause])
fft_edata         = np.abs(2.0/(end_pause-start_pause) * (fft_edata))[1:(end_pause-start_pause)//2]


# rf_data     	= sosfiltfilt(sos_df_band, rfsignal)
# rf_data     	= sosfiltfilt(sos_mains_stop, rf_data)
# rf_data     	= sosfiltfilt(sos_mains_stop2, rf_data)
rf_data     	= sosfiltfilt(sos_spike_band, rfsignal)

fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(111)
plt.plot(ffrequencies,fft_first_data,'k')
plt.plot(sfrequencies,fft_second_data,'r')
plt.show()
# 
# time_difference = 2.6 - 3.293
# 
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(211)
# plt.plot(t,envelope_signal,'k')
# plt.plot(t,envelope_signal_lp,'r')
plt.plot(t,rfsignal,'k')

ax2 = fig.add_subplot(212)
plt.plot(frequencies,fft_rfdata,'k')
# plt.plot(t,rf_data,'r')
...
plt.plot(frequencies,fft_edata,'r')
plt.plot(frequencies,100*fft_rfdata,'b')
